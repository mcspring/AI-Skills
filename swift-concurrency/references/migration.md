...
Replace manual `Task.sleep` debouncing with `AsyncAlgorithms.debounce()` for automatic cancellation and backpressure handling.

> See [async-algorithms.md § debounce](async-algorithms.md#debouncefortoleranceclock) for the full before/after pattern with SwiftUI integration.
...
## Linting & Suppression Strategies
...
2. If `async` is required (protocol/override/@concurrent), narrowly suppress:
...
**Fix when**: the warning identifies a real data race risk, the fix is straightforward, or the code is new.

**Suppress when**: protocol/inheritance requires the signature, third-party code forces the pattern, or migration is in progress (with tracked ticket).

### Suppression Annotations

```swift
@preconcurrency import LegacyFramework          // legacy imports
nonisolated(unsafe) var legacyCallback: (() -> Void)?  // single declaration
